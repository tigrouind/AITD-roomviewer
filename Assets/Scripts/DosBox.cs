...
public class DosBox : MonoBehaviour
{
	...
	public bool LinkToDosBOX(int floor, int room)
	{
		...
		{
			ProcessHelper procMemory = ProcessHelper.OpenProcess(processId);
			for (int patterns = 0; patterns < PlayerInitialPosition.Length; patterns++)
			{
				foreach(var pattern in PlayerInitialPosition[patterns])
				{
					long address = procMemory.SearchForBytePattern(pattern);
					if (address != -1)
					{
						//force reload
						linkfloor = floor;
